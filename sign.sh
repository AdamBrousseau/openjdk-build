...
signRelease()
{
  case "$OPERATING_SYSTEM" in
    "windows")
      ...
      do
        echo "Signing ${f}"
        if ! "$signToolPath" sign /f "${SIGNING_CERTIFICATE}" /p "$SIGN_PASSWORD" /fd SHA256 /t http://timestamp.digicert.com/ "$f"; then
          echo "RETRYWARNING: Failed to sign ${f} at $(date +%T): Possible timestamp server error - RC $? ... Retrying in 10 seconds"
          sleep 10s
          "$signToolPath" sign /f "${SIGNING_CERTIFICATE}" /p "$SIGN_PASSWORD" /fd SHA256 /t http://timestamp.digicert.com/ "$f"
        fi        
      done
